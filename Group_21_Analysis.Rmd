...
b1<-ggplot(data = group21_new, aes(x = newprice, y = depth , fill = newprice))+
  geom_boxplot() +
  labs(x = "More or Less", y = "Depth", title = "Plot1:Newprice by depth")+ 
  theme(legend.position = "none")

b2<-ggplot(data = group21_new, aes(x = newprice, y = height , fill = newprice))+
  geom_boxplot() +
  labs(x = "More or Less", y = "Height", title = "Plot2:Newprice by height")+ 
  theme(legend.position = "none")

b3<-ggplot(data = group21_new, aes(x = newprice, y = width , fill = newprice))+
  geom_boxplot() +
  labs(x = "More or Less", y = "Width",title = "Plot3:Newprice by width")+ 
  theme(legend.position = "none")
...
```{r barplot1, warning=FALSE, message=FALSE, fig.cap="\\label{bar1:Newprice} boxplot of newprice by categorical vairalbes"}
#Barplot
group21_new %>%
...
bar1<-ggplot(data = group21_new, aes(x = newprice, group = category)) +
  geom_bar(aes(y = after_stat(prop), fill = category), stat = "count", position = "dodge") +
  labs(x = "More or Less", y = "Proportion")
bar1
```
Figure \ref{bar1:Newprice}

```{r barplot2, warning=FALSE, message=FALSE, fig.cap="\\label{bar3:Newprice} boxplot of newprice by categorical vairalbes"}
group21_new %>%
  tabyl(newprice, sellable_online) %>%
  adorn_percentages() %>%
  adorn_pct_formatting() %>%
  adorn_ns() # To show original counts
bar2<-ggplot(data = group21_new, aes(x = newprice, group = sellable_online)) +
  geom_bar(aes(y = after_stat(prop), fill = sellable_online), stat = "count", position = "dodge") +
  labs(x = "More or Less", y = "Proportion")
bar2
```
Figure \ref{bar2:Newprice}

```{r barplot3, warning=FALSE, message=FALSE, fig.cap="\\label{bar3:Newprice} boxplot of newprice by categorical vairalbes"}
group21_new %>%
  tabyl(newprice, other_colors) %>%
  adorn_percentages() %>%
  adorn_pct_formatting() %>%
  adorn_ns() # To show original counts
bar3<-ggplot(data = group21_new, aes(x = newprice, group = other_colors)) +
...
model_1<- glm(newprice ~ category+sellable_online+other_colors+depth+height+width, data = group21_new, family = binomial(link = "logit"))
...
There is a categorical variables, `category`, is the model, in order to check which category fits the model well, p-value is checked below.
...
levels(data_12$newprice)
```

From the result we know that the baseline category for our binary response is `more`. Meaning that Estimates from the logistic regression model represent a shift on the log-odds scale for `more` compared to the baseline response.

```{r log_odds, warning=FALSE, message=FALSE}
#log_odds
modcoefs <- round(coef(model_4), 2)
confint(model_4) %>%
  kable()
```

$$\ln(\displaystyle\frac{p}{1-p})=\alpha+\beta_{1}x_{i}+ \beta_{2}\textrm{depth}+ \beta_{3} \textrm{depth}$$ ,where $x_{i}$ is the ith category.
...
```{r odds, warning=FALSE, message=FALSE}
#odds
model_4 %>%
  coef() %>%
  exp()
...
$\displaystyle\frac{p}{1-p}=exp(\alpha+\beta_{1}x_{i}+ \beta_{2}\textrm{depth}+ \beta_{3} \textrm{depth})$ ,where $x_{i}$ is the ith category.

```{r, warning=FALSE, message=FALSE}
plot_model(model_4, show.values = TRUE, axis.lim = c(1,1.5),
           title = "Odds (less)", show.p = FALSE)
```

```{r probability, warning=FALSE, message=FALSE}
