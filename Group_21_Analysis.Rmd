...
confint(model_4) %>%
  kable()
```
$$\ln(\displaystyle\frac{p}{1-p})=\alpha+\beta_{1}x_{i}+ \beta_{2}\textrm{depth}+ \beta_{3} \textrm{depth}$$
,where $x_{i}$ is the ith category.
...
model_4 %>%
  ...
  exp()
```
$\displaystyle\frac{p}{1-p}=exp(\alpha+\beta_{1}x_{i}+ \beta_{2}\textrm{depth}+ \beta_{3} \textrm{depth})$
,where $x_{i}$ is the ith category.
```{r, warning=FALSE, message=FALSE}
